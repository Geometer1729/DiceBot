...
import TypeCheckerCore (DType (..), ExprT (Hask), HRefable (..), HsOf, Ref, Res (Res), sRef)

castPolly ::
  forall h.
  (SingI (DOf h), Just0 (DOf h)) =>
  (forall d. HsOf (Ref 0 d (DOf h))) ->
  Res
...
buildPoly ::
  forall h.
  (SingI (DOf h), Just0 (DOf h)) =>
  (forall d. Proxy d -> HsOf (Ref 0 d (DOf h))) ->
  ExprT (DOf h)
